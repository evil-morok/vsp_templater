...
import {processYamlFile, Section} from './parse_yaml';

let sectionDataProvider: SectionTreeDataProvider;

export class SectionTreeDataProvider implements
    vscode.TreeDataProvider<SectionItem> {
  private _onDidChangeTreeData:
      vscode.EventEmitter<SectionItem|undefined|null|void> =
  ...
  constructor(root: Section) { this.root = root; }

  getTreeItem(element: SectionItem): vscode.TreeItem {
    const treeItem = new vscode.TreeItem(
        element.name, element.isLeaf
                          ? vscode.TreeItemCollapsibleState.None
                          : vscode.TreeItemCollapsibleState.Collapsed);

    // If it's a leaf node, we can display additional section data in the
    // tooltip or description
    if (element.isLeaf) {
      treeItem.tooltip =
          `Section Data: ${JSON.stringify(element.section, null, 2)}`;
  ...
  private createSubsections(section: Section): SectionItem[] {
    ...
    Object.keys(section).forEach(key => {
      if (key !== "name" &&
          section[key]
              ?.name) { // Ensure the section has a "name" before adding to tree
        const subsection: SectionItem = {
          name : section[key].name, // Only add the name if it exists
          section : section[key],
          isLeaf : !this.hasChildren(
              section[key]), // Mark it as a leaf if it has no children
        };
  ...
  private hasChildren(section: Section): boolean {
    ...
    return Object.keys(section).some(key =>
                                         key !== "name" && section[key]?.name);
  }

  // Add a refresh function to trigger a tree update
  refresh(): void {
    this._onDidChangeTreeData
        .fire(); // This will notify VSCode to refresh the tree view
  }
...
export function activate(context: vscode.ExtensionContext, filePath: string) {
  ...
  vscode.window.registerTreeDataProvider('vsp_templaterView',
                                         sectionDataProvider);
  ...
  context.subscriptions.push(vscode.commands.registerCommand(
      'vs-templater.applyTemplate', async (item: SectionItem) => {
        vscode.window.showInformationMessage(
            `Executing action for: ${item.name}`);
        ...
        if (item.section.file_mappings) {
          ...
          for (const key in item.section.file_mappings) {
            const inputFile = path.join(workspaceFolders[0].uri.fsPath, key);
            const outputFile = path.join(workspaceFolders[0].uri.fsPath,
                                         item.section.file_mappings[key]);

            try {
              await parseTemplate(inputFile, outputFile,
                                  {...item.section.substitutions});
            } catch (error) {
              hasErrors = true;
              errors.push(`${key}: ${
          ...
          if (!hasErrors) {
            ...
            vscode.window.showErrorMessage(`Failed to process ${
                errors.length} template(s):\n${errors.join('\n')}`);
          }
        }
      }));
}
